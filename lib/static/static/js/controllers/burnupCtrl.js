    ...
    function burnupCtrl($routeParams, $scope, burnupCalculator, velocityMetrics, gitHubService, availabilityService, LABEL, $q) {
        ...
        function setPastMilestoneDataFrom(issues, milestone) {
            ...
            milestone.metadata.categories = {
                freerange: findCategory(LABEL.FREERANGES, categories),
                firelane: findCategory(LABEL.FIRELANES, categories),
                escalation: findCategory(LABEL.ESCALATIONS, categories),
                nearMiss: findCategory(LABEL.NEAR_MISSES, categories),
                build: findCategory(LABEL.BUILD_MACHINE, categories),
                zeroDefect: findCategory(LABEL.ZERO_DEFECTS, categories),
                feature: findCategory(LABEL.FEATURES, categories),
                research: findCategory(LABEL.RESEARCH, categories),
                technicalDebt: findCategory(LABEL.TECHNICAL_DEBT, categories)
            };
            
            // MOVE THIS TO VELOCITY METRICS?
